...
gulp.task('bower:insertfiles', function(cb) {
    return gulp.src(config.baseDirRoot + 'index.php') //file with tags for injection
        .pipe(inject(gulp.src([config.distCSS + '/**/**/*.css', config.publicJS + '/*.js', config.distCSS + '/**/**/*.js', config.publicCSS + '/*.css'], { read: false }), {
            starttag: '<!-- bower:{{ext}} -->',
            endtag: '<!-- endbower -->',
            relative: true
        }))
...
gulp.task('copy-bower-dep', function() {
    ...
    gulp.src(bowerMain('css', 'min.css').minified, { base: './bower_components' })
